...
// 使用示例
async function main() {
  const lrImage = await loadPNG("./data/valid/test.png");
  const [H_lr, W_lr, H_sr, W_sr] = getImageMetadata(lrImage);

  const srImage = await superResolve(
    "./model/1e-4-20",
    lrImage,
    H_lr,
    W_lr,
  ...
  fs.writeFileSync("./data/valid/sr_image.png", buffer);

  // 清理内存
  tf.dispose([lrImage, srImage]);
}
/* superResolve("../model/1e-4-20", lrImage, H_lr, W_lr, H_sr, W_sr).then(
  (srImage) => {
    tf.node.encodePng(srImage).then((buffer) => {
      fs.writeFileSync("./data/valid/sr_image.png", buffer);
    });
  }
);
